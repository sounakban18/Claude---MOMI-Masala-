...
## Latest fix: Save was silently behaving like Share

**Root cause:** the shared delivery function used by Save tried
`navigator.share()` **first**, before ever attempting a real download.
On any device where the Web Share API happens to be available (most
real phones), tapping "Save as PNG" therefore opened the native share
sheet instead of downloading a file — which is exactly "Save became
Share." Fixed by splitting delivery into two fully independent
functions in `js/utils/file-pipeline.js`:

- **`saveToDevice()`** — used only by Save. Never calls
  `navigator.share()` under any circumstance; only ever attempts a
  real download (anchor+blob → new-tab blob → new-tab data URI).
- **`shareOnly()`** — used only by Share. Never falls back to a
  download; only ever opens the native share sheet.

**Verified, not assumed:** wrote a test that fakes a fully-working
`navigator.share()` (so it would succeed if called) and confirmed Save
still produces a real downloaded file and never touches `share()` at
all — and conversely that Share never triggers a download or opens the
Save menu. Filenames are now `MOMI-MASALA-Rate-Card.{pdf,png,jpg}`.
Console logging was added at each export stage (Rendering → Canvas
generated → Blob created → File created → Save/Share completed via
method X) for easier debugging if a real device still has trouble.

This was a scoped, surgical fix — only `js/utils/file-pipeline.js` and
`js/export.js` were touched. Everything else (theme system, product
cards, layout, animations, branding, data) is unchanged from the
previous round.
